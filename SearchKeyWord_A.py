...
def getcfg(filename):
    try:
        with open(filename, 'r') as opencfg:
            cfglist = opencfg.readlines()
            a = []
        ...
        print(e)
        print('Something configured wrong, please check config file.')
...
cfgdict = getcfg('SearchKeyWord.cfg')
...
if __name__ == '__main__':
    while True:
        ...
        article = selectarticle(aid, kind, cfgdict['database'], cfgdict['user'], cfgdict['password'], cfgdict['host'],
                                cfgdict['port'])
        ...
        print(' ')
        print(' Title length:', len(article['title']))
